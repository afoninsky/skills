---
name: design-steward
description: Legacy monolithic design-engagement workflow retained for explicitly requested Design Steward work and resuming existing Design Steward engagements. Use only when the user names design-steward or supplies its existing Owner Design Brief, steward-state, or G0-G6 records. For new UI/UX research, direction, prototyping, implementation, protected changes, or reviews across web and mobile, use the product-design entrypoint. Do not auto-route new design commissions here.
...
> **Legacy explicit workflow:** keep using this skill for an existing Design Steward engagement or when the user explicitly requests its monolithic G0–G6 method. Route new product-design work through `product-design`.
