# Third-party skill notices for the archived Design Steward

The deprecated Design Steward composed two optional repository-local skills. Their instructions were subordinate to its clean-room, authority, and evidence contracts.

## grilling / grill-me runtime composition

Design Steward composed an existing installed skill named `grilling` or `grill-me` for the general one-question-at-a-time decision-tree interaction used during design-intent grilling. The public `SKILL.md` package format has no supported nested-skill dependency metadata, and this repository did not vendor or rewrite that skill.
...
The upstream skill directory does not contain a separate license file. This notice preserves the upstream repository's license declaration and exact source revision. Machine-readable provenance is in `references/specialist-provenance.json`.
