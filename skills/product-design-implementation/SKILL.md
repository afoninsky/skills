...
5. **Implement the smallest coherent slice.** Cover the entry context, core action, consequence, and relevant loading, empty, error, disabled, focus/selected, permission, long-content, localization, and recovery states. Include only states that belong to the requested surface or are necessary to preserve dependent behavior. Translate each interaction and accessibility requirement into observable platform semantics, state, behavior, and a targeted check; visible copy alone does not prove an announcement, relationship, or state transition.
